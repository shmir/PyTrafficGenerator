...
from kioxia_lab.vsphere_helper import datacenter_helper


def get_network_backing(client, porggroup_name, datacenter_name, portgroup_type):
    """
    ...
    filter = Network.FilterSpec(datacenters=set([datacenter]), names=set([porggroup_name]), types=set([portgroup_type]))
    network_summaries = client.vcenter.Network.list(filter=filter)

    if len(network_summaries) > 0:
        network = network_summaries[0].network
        print("Selecting {} Portgroup Network '{}' ({})".format(portgroup_type, porggroup_name, network))
        return network
    else:
        print("Portgroup Network not found in Datacenter '{}'".format(datacenter_name))
        return None
